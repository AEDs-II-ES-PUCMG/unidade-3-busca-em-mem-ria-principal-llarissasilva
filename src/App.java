...
public class App {
    ...
    */
    ...
    static <K> AVL<K, Produto> lerClientes(String nomeArquivoDados, Function<Produto, K> extratorDeChave) {
    	
    	Scanner arquivo = null;
    	int numClientes;
    	String linha;
    	AVL<K, Produto> clientesCadastrados;
        Cliente cliente;
    	...
    	try {
    		...
    		numClientes = Integer.parseInt(arquivo.nextLine());
    		clientesCadastrados = new AVL<K, Cliente>();
    		
    		for (int i = 0; i < numClientes; i++) {
    			linha = arquivo.nextLine();
    			cliente = Cliente.criarDoTexto(linha);
    			K chave = extratorDeChave.apply(cliente);
    ...
    public static void pedidosDoCliente() {
        ...
        int docCliente = teclado.nextInt();
        Cliente escolhido = clientesPorId.pesquisar(docCliente);
        List<Pedido> pedidosCliente = pedidosDoCliente();
        StringBuilder mensagem = new StringBuilder();
        mensagem.append("Cliente: "+ escolhido.getNome());
        mensagem.append("Histórico: \n" + pedidoCliente.toString());
        System.out.println();
    }
